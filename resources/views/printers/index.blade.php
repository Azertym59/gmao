    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                @endphp
                @if($defaultPrinter)
                    <p class="text-sm text-gray-300 mt-1">Imprimante par défaut : <span class="font-medium text-green-300">{{ $defaultPrinter->name }}</span></p>
                @else
                    <p class="text-sm text-yellow-300 mt-1">Aucune imprimante définie par défaut</p>
                @endif
            </div>
            <div class="flex space-x-2">
                <a href="{{ url('/etiquettes/test') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition-all duration-300 flex items-center">
                    ...
                    @if(count($printers) > 0)
                        ...
                        <tr class="border-b border-gray-700 hover:bg-gray-800/40 transition-colors">
                            ...
                            <td class="px-6 py-4 flex space-x-2">
                                <a href="{{ route('printers.edit', $printer->id) }}" class="p-2 bg-blue-500/20 text-blue-300 rounded-lg hover:bg-blue-500/40 transition-colors" title="Modifier l'imprimante">
                                ...
                                <a href="{{ route('printers.test') }}?printer_id={{ $printer->id }}" class="p-2 bg-green-500/20 text-green-300 rounded-lg hover:bg-green-500/40 transition-colors" title="Tester l'imprimante">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </a>
                                <a href="{{ url('/etiquettes/test?printer_id=' . $printer->id) }}" class="p-2 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/40 transition-colors" title="Tester les étiquettes">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" />
                                    </svg>
                                </a>
                                @if(!$printer->is_default)
                                <form method="POST" action="{{ route('printers.set-default', $printer->id) }}">
                                    @csrf
